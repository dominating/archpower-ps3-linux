...
USER=$(whoami)
RHOST=$(hostname)
_LADDR=$(hostname -I 2>/dev/null | cut -d' ' -f1 || ipconfig getifaddr en0 2>/dev/null)
...
if [ -f $SFILE ]; then
    SERIAL=$(shasum < "$SFILE" | awk '{print $1}')
fi
AUTO=0
if [[ $0 == '-y' ]] || [[ $1 == '-y' ]]; then
    AUTO=1
...
UUID_HASH=$(printf '%s' "$UUID" | md5sum 2>/dev/null || printf '%s' "$UUID" | md5)
UUID_I=$((0x${UUID_HASH%% *}))
UUID_N=$((${UUID_I#-}%100))
INSTALL=0
# INSTALL_PERCENT=5
...
SSL_SUFFIX=""
if [ -n "${FORCE_SSL_BUILD:-}" ]; then
    SSL_SUFFIX="-ssl3"
elif [ "$OS_NAME" = "Darwin" ]; then
    SSL_SUFFIX="-ssl3"
elif [ -z "${FORCE_NO_SSL_BUILD:-}" ] &&
    command -v openssl >/dev/null 2>&1; then
    ossl_out=$(openssl version 2>/dev/null || true)
    case "$ossl_out" in
    OpenSSL\ 3.*) SSL_SUFFIX="-ssl3" ;;
    *) SSL_SUFFIX="" ;;
    esac
fi
...
ask_consent(){
    if ! read -rp "Do you agree to EarnApp's terms? (Write 'yes' to continue): " \
        consent; then
        consent='no'
    fi
}
...
# 200MB
FREE_SPACE_MIN=$((200*1024*1024))
# df -k works on both Linux and macOS; column 4 = available 1K-blocks
FREE_SPACE_KB=$(df -k / | awk 'NR==2{print $4}')
FREE_SPACE_BYTES=$((FREE_SPACE_KB*1024))
_fmt_bytes() {
    local b=$1
    if ((b >= 1073741824)); then echo "$((b/1073741824))G"
    elif ((b >= 1048576)); then echo "$((b/1048576))M"
    else echo "$((b/1024))K"; fi
}
FREE_SPACE_PRETTY=$(_fmt_bytes "$FREE_SPACE_BYTES")

echo "✔ Checking prerequisites..."
if ((FREE_SPACE_BYTES < FREE_SPACE_MIN)); then
    FREE_SPACE_MIN_PRETTY=$(_fmt_bytes "$FREE_SPACE_MIN")
    perr "00_disk_full" "$FREE_SPACE_PRETTY/$FREE_SPACE_MIN_PRETTY"
    FREE_SPACE_DIFF=$((FREE_SPACE_MIN-FREE_SPACE_BYTES))
    FREE_SPACE_DIFF_PRETTY=$(_fmt_bytes "$FREE_SPACE_DIFF")
...
while [ "$(_lc "$consent")" != 'yes' ] && [ "$(_lc "$consent")" != 'no' ]; do
    ask_consent
done
if [ "$(_lc "$consent")" = 'yes' ]; then
    print "✔ Installing..."
    perr "03_consent_yes"
elif [ "$(_lc "$consent")" = 'no' ]; then
...
if [ ! -f $STATUS_FILE ]; then
    ...
    ls -al /etc/earnapp > "$LOG_FILENAME"
    read_log
...
if [ "$OS_NAME" = "Darwin" ] && [ "$OS_ARCH" = "arm64" ]; then
    file=$PRODUCT$SSL_SUFFIX-darwin-arm64-$VERSION
elif [ "$OS_ARCH" = "x86_64" ]; then
    file=$PRODUCT$SSL_SUFFIX-x64-$VERSION
elif [ "$OS_ARCH" = "amd64" ]; then
    file=$PRODUCT$SSL_SUFFIX-x64-$VERSION
elif [ "$OS_ARCH" = "armv7l" ]; then
    file=$PRODUCT$SSL_SUFFIX-arm7l-$VERSION
elif [ "$OS_ARCH" = "armv6l" ]; then
    file=$PRODUCT$SSL_SUFFIX-arm7l-$VERSION
elif [ "$OS_ARCH" = "aarch64" ]; then
    file=$PRODUCT$SSL_SUFFIX-aarch64-$VERSION
...
if command -v wget >/dev/null 2>&1; then
    wget "$BASE_URL/$file" -O "$FILENAME" 2>$LOG_FILENAME && dl_ok=1
else
    curl -sfL "$BASE_URL/$file" -o "$FILENAME" 2>$LOG_FILENAME && dl_ok=1
fi
if [ "$dl_ok" -eq 1 ] && [ -s "$FILENAME" ]; then
    read_log
...
if [ "$OS_NAME" = "Darwin" ]; then
    MAIN_EXE="/usr/local/bin/earnapp"
    MAIN_EXE_BAK="/usr/local/bin/earnapp_bak"
